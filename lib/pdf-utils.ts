...
export function generatePDFReport(data: PDFReportData): void {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    pdf.text('This report is generated by CrackCheck AI for reference only. Consult a professional structural engineer for critical issues.', 
    ...
    pdf.setFontSize(8)
    pdf.setFont('helvetica', 'bold')
    pdf.text('CrackCheck', margin, pageHeight - 5)
  }

  // Generate filename with proper naming
  const timestamp = data.timestamp.toISOString().slice(0, 10)
  const filename = `CrackCheck-Analysis-Report-${timestamp}-${data.timestamp.getTime().toString().slice(-6)}.pdf`
